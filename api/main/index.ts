...
serve(async (req: Request) => {
  ...
  const defaultWorkerConfig: ConfigWorker = {
    servicePath,
    ...
    restart: configuration.defaultWorkerConfig.restart, // default don't restart
  };

  const nowDate = new Date();
  const expireDate = new Date(
    nowDate.getTime() + defaultWorkerConfig.lifeTimeMs - 500
  );
  defaultWorkerConfig.expireDate = expireDate;
  ...
  */
  if (pathname === "/api/workerpool/add_worker" && req.method === "POST") {
    try {
      ...
      statSP.current_count_workers = countWorkersSP;
  ...
  const callWorker = async () => {
    try {
      ...
      if (workersServicePath === undefined || workersServicePath.length == 0) {
        const expireWorker = await createWorker(defaultWorkerConfig);
        const workers = [];
        workers.push(expireWorker);
        workerPool.set(servicePath, workers);
      }
